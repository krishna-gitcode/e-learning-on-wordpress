...
function cppm_render_purchased_grid( $filter_type = 'course' ) {
    ...
    if ( empty( $filtered_products ) ) {
        ...
        foreach ( $filtered_products as $product_id => $status ) {
            ...
            // NEW DYNAMIC ROUTING: Reverse Database Lookup to find the True Engine ID
            global $wpdb;
            
            if ( $filter_type === 'ebook' ) {
                // Find the internal E-Book ID connected to this Product
                $actual_id = $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = '_cppm_ebook_required_product' AND meta_value = %s LIMIT 1", $product_id ) );
                $course_link = $actual_id ? home_url( '/ebook-reader/?ebook_id=' . $actual_id ) : get_permalink( $product_id );
            } else {
                // Find the internal Course Playlist ID connected to this Product
                $actual_id = $wpdb->get_var( $wpdb->prepare( "SELECT post_id FROM {$wpdb->postmeta} WHERE meta_key = '_cppm_required_product' AND meta_value = %s LIMIT 1", $product_id ) );
                $course_link = $actual_id ? home_url( '/course-player/?course_id=' . $actual_id ) : get_permalink( $product_id );
            }
