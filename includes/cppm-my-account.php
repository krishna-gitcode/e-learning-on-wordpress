...
// ==========================================
// 6. PASSWORD RESET POP-UP (POST-REDIRECT)
// ==========================================
add_action('wp_footer', 'cppm_password_reset_popup_script');
function cppm_password_reset_popup_script() {
    // Only fire the popup IF the URL confirms the reset link was actually sent
    if ( ! is_user_logged_in() && isset($_GET['reset-link-sent']) && $_GET['reset-link-sent'] == 'true' ) {
        ?>
        <script>
        document.addEventListener('DOMContentLoaded', function() {
            var popupHTML = `
                <div id="cppm-pwd-popup" style="position:fixed; top:0; left:0; width:100%; height:100%; background:rgba(0,0,0,0.6); z-index:99999; display:flex; align-items:center; justify-content:center; font-family:'Jost', sans-serif;">
                    <div style="background:#fff; padding:40px; border-radius:12px; max-width:400px; text-align:center; box-shadow:0 10px 30px rgba(0,0,0,0.2); animation:cppmPop 0.3s ease-out; position:relative;">
                        <button onclick="document.getElementById('cppm-pwd-popup').remove()" style="position:absolute; top:15px; right:15px; background:none; border:none; font-size:24px; cursor:pointer; color:#94a3b8; transition:0.2s;">&times;</button>
                        <div style="width:60px; height:60px; background:#dcfce7; border-radius:50%; display:flex; align-items:center; justify-content:center; margin:0 auto 20px;">
                            <svg width="30" height="30" viewBox="0 0 24 24" fill="none" stroke="#166534" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M22 11.08V12a10 10 0 1 1-5.93-9.14"></path><polyline points="22 4 12 14.01 9 11.01"></polyline></svg>
                        </div>
                        <h3 style="margin:0 0 10px; font-size:22px; color:#0f172a;">Reset Link Sent!</h3>
                        <p style="color:#64748b; margin:0 0 20px; font-size:15px;">We have sent a password reset link to your email address.</p>
                        <p style="color:#94a3b8; font-size:13px; margin:0;">Please check your spam/junk folder if you don't see it.</p>
                        <button onclick="document.getElementById('cppm-pwd-popup').remove()" style="margin-top:20px; background:#2874f0; color:#fff; border:none; padding:10px 20px; border-radius:6px; cursor:pointer; font-weight:bold; width:100%;">Return to Login</button>
                    </div>
                </div>
                <style>@keyframes cppmPop { from { transform:scale(0.9); opacity:0; } to { transform:scale(1); opacity:1; } }</style>
            `;
            
            // Hide the default ugly WooCommerce text message
            var wcMessage = document.querySelector('.woocommerce-message');
            if (wcMessage) wcMessage.style.display = 'none';

            // Show our beautiful pop-up
            document.body.insertAdjacentHTML('beforeend', popupHTML);
            
            // Silently clean up the URL so refreshing the page doesn't trigger the pop-up again
            if (window.history && window.history.replaceState) {
                const url = new URL(window.location);
                url.searchParams.delete('reset-link-sent');
