...
function cppm_render_ebook_reader( $atts ) {
    // 1. DYNAMIC ROUTING: Check URL First
    $ebook_id = 0;
    if ( isset($_GET['ebook_id']) && !empty($_GET['ebook_id']) ) {
        $ebook_id = intval($_GET['ebook_id']);
    } else {
        // 2. FALLBACK: Check Shortcode (Bulletproof Array Check)
    ...
    if ( $ebook_id === 0 ) {
        return '<div style="padding:40px; text-align:center; background:#f8fafc; border-radius:12px; border:1px solid #e2e8f0;"><h3 style="color:#0f172a; margin-top:0;">E-Book Not Found</h3><p>Missing E-Book ID in URL or Shortcode.</p></div>';
    }
    ...
    document.addEventListener("DOMContentLoaded", function() {
        ...
        function loadPDF(dIdx, targetPage = null) {
            ...
            // FIX: Prevent PDF Memory Overload by disabling AutoFetch
        ...
        // FIX: iOS Bulletproof Fullscreen logic
        document.getElementById('fs-btn-' + uid).addEventListener('click', function() {
            if (wrap.classList.contains('cppm-ios-fullscreen')) {
                // Exit iOS fallback
                wrap.classList.remove('cppm-ios-fullscreen');
                ...
                if (wrap.requestFullscreen) {
                    ...
                    /* iOS iPhone Fallback */
                    wrap.classList.add('cppm-ios-fullscreen');
