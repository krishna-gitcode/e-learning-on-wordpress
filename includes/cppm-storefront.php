...
// ==========================================
// 4. STOREFRONT: ADD TO CART / BUY NOW INJECTOR
// ==========================================
add_filter( 'woocommerce_loop_add_to_cart_link', 'cppm_custom_loop_buttons', 10, 3 );
function cppm_custom_loop_buttons( $html, $product, $args ) {
    $is_variable = $product->is_type('variable');
    
    // Cart Icon
    $cart_icon = '<svg class="cppm-storefront-cart-icon" width="20" height="20" viewBox="0 0 24 24" fill="none" stroke="#374151" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><circle cx="9" cy="21" r="2"></circle><circle cx="20" cy="21" r="2"></circle><path d="M1 1h4l2.68 13.39a2 2 0 0 0 2 1.61h9.72a2 2 0 0 0 2-1.61L23 6H6"></path><line x1="16" y1="10" x2="16" y2="2"></line><line x1="12" y1="6" x2="20" y2="6"></line></svg>';
    
    $add_to_cart_url = $product->add_to_cart_url();
    $classes = isset( $args['class'] ) ? $args['class'] : 'button';
    
    // Output the CSS only ONCE per page load to keep the HTML clean
    ...
    if ( ! $styles_printed ) {
        $style_block = '<style>
            ...
            /* --- MOBILE VIEW (Under 484px) --- */
            @media (max-width: 484px) {
                svg.cppm-storefront-cart-icon{
                    display:none !important;
                }
                .cppm-action-row {
                    flex-direction: column; /* Stacks the buttons top-to-bottom */
                    gap: 8px;
                }
                .cppm-cart-btn {
                    flex: none; /* Removes the 54px restriction */
                    width: 100%; /* Forces full card width */
                    height: 42px; /* Slightly slimmer for mobile */
                    gap: 8px; /* Adds space between icon and text */
                }
                .cppm-cart-text { 
                    display: block; /* Reveals the "Add to Cart" text */
                }
                .cppm-buy-btn {
                    flex: none;
                    width: 100%; /* Forces full card width */
    ...
    $output = $style_block . '<div class="cppm-action-row">';
    
    if ( $is_variable ) {
        $output .= '<a href="' . esc_url($product->get_permalink()) . '" class="cppm-store-btn cppm-buy-btn">Select Options</a>';
    } else {
        // 1. Add to Cart Button (Contains BOTH the icon and the text, CSS handles visibility)
        $cart_inner = $cart_icon . '<span class="cppm-cart-text">Add to Cart</span>';
        
        $output .= sprintf( '<a href="%s" data-quantity="%s" class="%s cppm-cart-btn" %s title="%s" style="color:#374151 !important; background:#ffffff !important;border:2px solid #374151 !important;">%s</a>',
        ...
        // 2. Buy Now Button
        $checkout_url = wc_get_checkout_url() . '?add-to-cart=' . $product->get_id() . '&buy_now=1';
        $output .= '<a href="' . esc_url($checkout_url) . '" class="cppm-store-btn cppm-buy-btn">Buy Now</a>';
    }
...
// ==========================================
// BUY NOW LOGIC (BULLETPROOF CART & REDIRECT)
// ==========================================

// Step 1: Empty the cart BEFORE adding the "Buy Now" item
add_filter( 'woocommerce_add_to_cart_validation', 'cppm_clear_cart_for_buy_now', 10, 3 );
function cppm_clear_cart_for_buy_now( $passed, $product_id, $quantity ) {
    if ( isset($_GET['buy_now']) && $_GET['buy_now'] == '1' ) {
        // If Buy Now was clicked, wipe out any old items sitting in the cart
        WC()->cart->empty_cart();
    }
    return $passed;
}

// Step 2: Redirect securely to checkout AFTER the item is added and saved
add_filter( 'woocommerce_add_to_cart_redirect', 'cppm_redirect_checkout_for_buy_now', 99 );
function cppm_redirect_checkout_for_buy_now( $url ) {
    if ( isset($_GET['buy_now']) && $_GET['buy_now'] == '1' ) {
        // Bypass the cart page entirely and go straight to payment
